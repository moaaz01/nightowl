...
# ─── Paths ────────────────────────────────────────────────────────────
# In a git clone, ROOT == repo root. When installed via pip/Docker, override
# where runtime artifacts land with NIGHTOWL_HOME (never write into
# site-packages). TARGETS stays relative to the current working directory.
ROOT = Path(__file__).resolve().parent.parent
DATA_DIR = Path(os.environ.get("NIGHTOWL_HOME") or ROOT)
TARGETS = Path(os.environ.get("NIGHTOWL_TARGETS") or Path.cwd() / "targets")
if not TARGETS.exists():
    TARGETS = DATA_DIR / "targets"
REPORTS = DATA_DIR / "workspace" / "reports"
