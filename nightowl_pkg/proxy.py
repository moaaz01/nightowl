# proxy.py -- NightOwl v8 Universal Traffic Capture & Proxy Integration
#
...
MITM_ADDON = '''"""
NightOwl v8 universal capture addon for mitmproxy/mitmweb/mitmdump.
...
def _ca(args):
    ...
    r = subprocess.run(
        ["openssl", "x509", "-inform", "PEM", "-subject_hash_old", "-noout",
         "-in", cert], capture_output=True, text=True, timeout=20)
    h = r.stdout.strip() or subprocess.run(
        ["openssl", "x509", "-inform", "DER", "-subject_hash_old", "-noout",
         "-in", cert], capture_output=True, text=True, timeout=20).stdout.strip()
    if not h:
...
<!-- Generated by NightOwl v8 -- trust user-installed CAs for authorized testing -->
